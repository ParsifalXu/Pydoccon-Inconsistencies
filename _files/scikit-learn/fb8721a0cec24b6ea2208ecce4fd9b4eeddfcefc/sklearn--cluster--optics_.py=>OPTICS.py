class OPTICS(BaseEstimator, ClusterMixin):
    ...
    def fit(self, X, y=None):
        ...
        self.ordering_, self.core_distances_, self.reachability_, self.predecessor_ = compute_optics_graph(X=X, min_samples=self.min_samples, algorithm=self.algorithm, leaf_size=self.leaf_size, metric=self.metric, metric_params=self.metric_params, p=self.p, n_jobs=self.n_jobs, max_eps=self.max_eps)
        if self.cluster_method == 'xi':
            labels_, clusters_ = cluster_optics_xi(self.reachability_, self.predecessor_, self.ordering_, self.min_samples, self.min_cluster_size, self.xi, self.predecessor_correction)
            ...
            labels_ = cluster_optics_dbscan(self.reachability_, self.core_distances_, self.ordering_, eps)
